...
// DISCLAIMER: half the time I can actually understand what's happening, other half of the time I have no clue, so I don't claim any credit whatsoever for creating this. This was just a fun experiment for me, but don't ask me to try and explain it.

onmessage = function (e) {
  const data = e.data;
  let result = {};
  if (data.salt) {
    result.hash = hash(data.msg + data.salt);
    postMessage(result);
    return;
  }
  const salted = salt(data.msg);
  result.hash = hash(salted.saltedMsg);
  result.salt = salted.salt;
  postMessage(result);
  return;
};
